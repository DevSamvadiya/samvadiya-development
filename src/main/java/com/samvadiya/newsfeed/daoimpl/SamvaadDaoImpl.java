...
public class SamvaadDaoImpl implements SamvaadDao {
	...
	if (jdbcTemplate.getDataSource() == null)
	    throw new Exception("Data Source Not Available");
	...
	ProcedureBuilder proc = new ProcedureBuilder(jdbcTemplate.getDataSource(), GET_SAMVAAD, sqlInParameters,
		sqlOutParameters, sqlResultParameters);
	...
	ProcedureBuilder proc = new ProcedureBuilder(jdbcTemplate.getDataSource(), GET_COMMENT, sqlInParameters,
		sqlOutParameters, sqlResultParameters);
	...
	Map<String, List<CommentModel>> result = list.stream()
		.collect(Collectors.groupingBy(x -> (x.getUserId() + x.getSamvaadId())));
	...
	sqlResultParameters.add(
		new StoreProcParameters("FRIEND_ACTIVITY_LIST", null, Types.REF_CURSOR, new FriendsActivityMapper()));
	sqlInParameters.add(new StoreProcParameters("IN_USER_ID", userId, Types.VARCHAR, null));
	sqlOutParameters.add(new StoreProcParameters("OUT_STATUS", null, Types.INTEGER, null));
	ProcedureBuilder proc = new ProcedureBuilder(jdbcTemplate.getDataSource(), GET_FRIENDS_ACTIVITY,
		sqlInParameters, sqlOutParameters, sqlResultParameters);
	Map<String, CommentModel> data = proc.execute(sqlInParameters);
	...
	ProcedureBuilder proc = new ProcedureBuilder(jdbcTemplate.getDataSource(), GET_DAILY_NEWS, sqlInParameters,
		sqlOutParameters, sqlResultParameters);
